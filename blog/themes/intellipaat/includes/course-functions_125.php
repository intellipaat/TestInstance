...
LEFT JOIN $wpdb->commentmeta cm ON (c.comment_ID=cm.comment_id) LEFT OUTER JOIN $wpdb->posts p ON (c.comment_post_ID =p.ID)
 WHERE p.post_type = 'course' AND c.comment_type = '' AND c.comment_approved = '1' AND p.ID=$course_id AND cm.meta_key='order_on_course' ORDER BY CONVERT(cm.meta_value, SIGNED INTEGER) ASC LIMIT 80";
...
LEFT JOIN $wpdb->commentmeta ON ($wpdb->comments.comment_ID=$wpdb->commentmeta.comment_id) LEFT OUTER JOIN $wpdb->posts ON ($wpdb->comments.comment_post_ID =$wpdb->posts.ID)
 WHERE $wpdb->posts.post_type = 'course' AND comment_type = '' AND comment_approved = '1' AND $wpdb->posts.ID=$course_id AND $wpdb->commentmeta.meta_key!='order_on_course' GROUP BY comment_author_email LIMIT 80";
	...
	$count = 0;
	$comment_unique = array();
	foreach($comments as $comment ){
		if (!in_array($comment->comment_ID, $comment_unique)){
		$comment_unique[] = $comment->comment_ID;
