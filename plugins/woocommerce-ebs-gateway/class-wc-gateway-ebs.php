...
Description: EBS Payment Gateway.
Version: 2.0
Author: EBS
Author URI: http://www.ebs.in
*/

add_action('plugins_loaded', 'woocommerce_ebs_init', 0);

function woocommerce_ebs_init() {
	if ( !class_exists( 'WC_Payment_Gateway' ) ) return;
	
	load_plugin_textdomain('wc-gateway-ebs', false, dirname( plugin_basename( __FILE__ ) ) . '/languages');
	
	class WC_Gateway_Ebs extends WC_Payment_Gateway {
		public function __construct() {
	        ...
	        $this->method_title = __( 'EBS', 'woocommerce' );
		    ...
		    // Actions
			add_action( 'woocommerce_api_wc_gateway_ebs', array( $this, 'check_ebs_response' ) );
	...
	function get_ebs_args( $order ) {
			...
			$amount = $this->conversion_rate  * $order->order_total;
			$mode = ($ebs_mode == "yes") ? "TEST" : "LIVE";
			$return_url = trailingslashit(home_url()).'?wc-api=WC_Gateway_Ebs&order_id='.$order_id;
	...
	$shipping_country = $Code[$order->shipping_country];
		...
		function check_ebs_response() {
				...
				$amount = $this->conversion_rate * $order->order_total;
				$amount = apply_filters( 'formatted_woocommerce_price',  number_format( $amount, 2 , '.', '' ), $amount, 2 , '.', '' );
				...
				if($response['ResponseCode']==0){				
					if($response['Amount'] == $amount){
						...
						wp_mail( "mane.makarand@gmail.com" , "Payment done. Order update failure. amount mismatch", $email."\r\n\r\n<pre>".print_r($_REQUEST, TRUE).print_r($order, TRUE).print_r($amount, TRUE).'</pre>' ); 
						$order -> add_order_note("The payment has been done but order failed to complete, due to total amount mismatch.");
	...
	function add_ebs_gateway($methods) {
		$methods[] = 'WC_Gateway_Ebs';
		return $methods;
	}
	
	add_filter('woocommerce_payment_gateways', 'add_ebs_gateway' );
}
