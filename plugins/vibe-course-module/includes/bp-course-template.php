...
function bp_course_get_course_meta() {
	...
	$meta ='<div itemscope itemtype="http://schema.org/Organization">';
	$meta .= bp_course_display_rating($reviews);

	$meta .= '( <strong itemprop="reviewCount">'.(($count)?$count:'0').'</strong> '.__('REVIEWS','vibe').' )</div></div>';
}
*/
function bp_course_get_course_meta() {
	...
	$count21 =get_comments_number(get_the_ID());


	$meta ='<div itemscope itemtype="http://schema.org/Organization">';
	$meta .= bp_course_display_rating($reviews);

	$meta .= '( <strong itemprop="reviewCount">'.(($count21)?$count21:'0').'</strong> '.__('REVIEWS','vibe').' )</div>';
	$meta .= ' </div>';
...
if(!function_exists('bp_course_display_rating')){
	function bp_course_display_rating($reviews){
		$meta = '<div class="star-rating" itemprop="aggregateRating" itemscope itemtype="http://schema.org/AggregateRating">
			<div style="display:inline-block;color:#000;" itemprop="ratingValue"></div>';
			for($i=1;$i<=5;$i++){
				if($reviews >= 1){
