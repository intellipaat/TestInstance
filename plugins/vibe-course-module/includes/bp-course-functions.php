...
function bp_course_add_user_to_course($user_id,$course_id,$duration = NULL){
  if(empty($duration))
    $duration=get_post_meta($course_id,'vibe_duration',true);
    ...
    $existing = get_user_meta($user_id,$course,true);
    if(empty($existing)){
      update_user_meta($user_id,'course_status'.$course,1);
    }else{
      update_user_meta($user_id,'course_status'.$course,2);
    }
