...
function intellipaat_course_offer($id){
	...
	$individual_course_offer_mode	= vibe_get_option('individual_course_offer_mode');
	$excluded_courses_from_offer 	= vibe_get_option('excluded_courses_from_offer');
	
	if($global_course_offer) 
		intellipaat_print_offer($global_course_offer);
		
	$course_offer = get_field('intellipaat_course_offer', $id); 
	if($course_offer && ($individual_course_offer_mode || (!$individual_course_offer_mode && in_array($id,$excluded_courses_from_offer))) ) 
  ...
  // Free Course
   $free_course= get_post_meta($course_id,'vibe_course_free',true);
	...
	if(!is_user_logged_in() && vibe_validate($free_course)){
		echo apply_filters('wplms_course_non_loggedin_user','<a href="#login-form" class="course_button button full popup-with-form">'.apply_filters('wplms_take_this_course_button_label',__('Take Self-Paced Course','vibe'),$course_id).'</a>'); 
		return;
	}
		  ...
		  echo '<a href="'.get_permalink($course_id).'" class="'.((isset($id) && $id )?'':'course_button full ').'button"  '.$rel.'>'.__('Start Online training','vibe').'</a>'; 
			...
			if(isset($pid) && $pid !='' && function_exists('get_product')){
				...
				if($post->ID==4028)
				{
						...
						<a href="'.$product_link.'" data-product_id="'.$pid.'" class="'.((isset($id) && $id )?'':' ').'button add_to_cart_button" onClick="ga(\'send\', \'event\', { eventCategory: \'button\', eventAction: \'click\', eventLabel: \'TAKE IBM CERTIFIED TRAINING\'});" '.$rel.'>'.__('TAKE IBM CERTIFIED TRAINING','vibe').apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>
						<div><a href="#" style="position:relative;">What IBM Certified Course contains?<div class="tool-tip left"><ul><li>In Addition to Big Data Hadoop training Course you will:</li><li>1) Undertake project work and will be allotted a 2-hour session with IBM Certified Trainer to discuss project solution.</li><li>2) IBM Certificate will be awarded on project completion.</li></ul></div></a></div>
						'; 
				}else{
					echo '<div itemprop="offers" itemscope="" itemtype="http://schema.org/Offer">
						<h5 class="credits price">'.$credits .'</h5>
						...
						<a href="'.$product_link.'" data-product_id="'.$pid.'" class="'.((isset($id) && $id )?'':' ').'button add_to_cart_button" onClick="ga(\'send\', \'event\', { eventCategory: \'button\', eventAction: \'click\', eventLabel: \'Take Online training\'});" '.$rel.'>'.__('Take Online training','vibe').apply_filters('wplms_course_button_extra',$extra,$course_id).'</a>'; 
				}
...
LEFT JOIN $wpdb->commentmeta cm ON (c.comment_ID=cm.comment_id) LEFT OUTER JOIN $wpdb->posts p ON (c.comment_post_ID =p.ID)
 WHERE p.post_type = 'course' AND c.comment_type = '' AND c.comment_approved = '1'  AND CONVERT(cm.meta_value, SIGNED INTEGER)>0 AND p.ID=$course_id AND cm.meta_key='order_on_course' ORDER BY CONVERT(cm.meta_value, SIGNED INTEGER) ASC LIMIT 80";
...
LEFT JOIN $wpdb->commentmeta ON ($wpdb->comments.comment_ID=$wpdb->commentmeta.comment_id) LEFT OUTER JOIN $wpdb->posts ON ($wpdb->comments.comment_post_ID =$wpdb->posts.ID)
 WHERE $wpdb->comments.comment_ID NOT IN (".$comments1_temp[0]->ids_va.") AND  $wpdb->posts.post_type = 'course' AND comment_type = '' AND comment_approved = '1' AND $wpdb->posts.ID=$course_id AND $wpdb->commentmeta.meta_key!='order_on_course' GROUP BY comment_author_email ORDER BY  comment_date_gmt DESC  LIMIT 80";
