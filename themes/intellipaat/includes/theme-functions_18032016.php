		...
		echo '<div id="browse_courses" class="cats-dropdown cats-toggle">';
			...
			echo '<div class="dropdown-menu" >';
				...
				foreach($cats as $cat){
					...
					$mainHref = site_url( '/all-courses/'.$term->slug );
					...
					$count = 0;
					echo '<li  data-submenu-id="submenu-'.$term->slug.'"><a class="main-cat menu-item" href="javascript:void(0)" title="'.$term->name.'" ><span>'.$term->name.' ('.$no_of_courses.')</span><i class="icon-arrow-1-right arr"></i></a>';
						//style="width:'.$box_width.'px"
						echo '<div id="submenu-'.$term->slug.'" class="dropdown-menu sub" >';
						echo '<h4 class="heading">All '.$term->name.' courses</h4>';
						foreach ( $courses as $course ) : 
							if($count%$no_of_courses_shown_in_cat == 0)
								echo '<ul class="browse-sub-menu">';
							if($count < 10){
							$dbTitle = get_the_title($course->ID);
							$titleLen = strlen($dbTitle);
							if($titleLen > 38){
								$trimTitle = substr($dbTitle,0,38).'...';
							}else{
								$trimTitle = $dbTitle;
							}
							...
							echo '<li>
									<a href="'.get_permalink($course->ID).'">'.$trimTitle.'</a>
								</li>'; 
							$count++;
							}
							...
							if($no_of_courses > 10 && $count == 10){
								echo "<div style='width:100%;text-align:center'><a href='".$mainHref."' class='button btn-info'>View All Courses</a></div>";
								break;
							}	
