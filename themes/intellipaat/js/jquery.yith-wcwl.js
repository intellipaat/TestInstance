jQuery( document ).ready( function( $ ){
     ...
     *	Added by Makarand 
	 **/
    $(document).on( 'click', '.add_course_to_cart', function( ev ) {
        var t = $( this);

        ev.preventDefault();

        call_add_course_to_cart( t );
     ...
     *	Added by Makarand 
	 **/
    $(document).on( 'click', '.add_to_cart_both', function( ev ) {
        var t = $( this);

        ev.preventDefault();

        call_add_to_cart_both( t );

        return false;
    } );
    $(document).on( 'added_to_cart', 'body', increment_cart_count );
	
	$(document).on('mouseleave','.add_to_cart_both_wrap',function(){
     ...
     *	Added by Makarand 
	 **/
    $(document).on( 'click', '.remove_course_from_wishlist', function( ev ){
        var t = $( this );

        ev.preventDefault();

        remove_courseitem_from_wishlist( t );
     ...
     *
	 *	Added by Makarand makarandmane.com
	 *
     * @param object el
     * @return void
     * @since 1.0.0
     */
	function call_add_course_to_cart( el ) {
		if( !$( '#yith-wcwl-popup-message' ).length ) {
			$( 'body' ).prepend(
				'<div id="yith-wcwl-popup-message" style="display:none;">' +
					'<div id="yith-wcwl-msg"></div>' +
					'</div>'
			);
		}
		...
		var elwrap = $( '.wishlist_items-' + course_id );
        ...
        $.ajax({
            ...
            success: function( response ) {
                ...
                if( response_result == true ) {
                    elwrap.find('.remove').trigger('click');
                } 

				$( '#yith-wcwl-msg' ).html( response_message );
				msg.css( 'margin-left', '-' + $( msg ).width() + 'px' ).fadeIn();
				window.setTimeout( function() {
					msg.fadeOut();
				}, 2000 );
     ...
     * @since 1.0.0
     */
	function call_add_to_cart_both( el ) {
		
        var course_id = el.data( 'course_id');
		
		$('.add_to_cart_both_wrap-'+course_id).toggle().toggleClass('hide');
	}
	function increment_cart_count( ) {		
        var count = parseInt($('.cart_count').text())+1;
		if(count == 1)
			$('.cart_count').parent('a').attr('href',intellipaat.cartUrl);
     ...
     * @since 1.0.0
     */
    function remove_courseitem_from_wishlist( el ) {
        var product_id = el.data( 'product-id');
		var elwrap = $( '.add-to-wishlist-' + product_id );
		
		data = {
			action: 'remove_course_from_wishlist',
        ...
        $.ajax({
            ...
            success: function( response ) {
                ...
                if( response_result == true ) {
                    ...
                    elwrap.find('.yith-wcwl-add-button').show().removeClass('hide').addClass('show');
                    elwrap.find( '.yith-wcwl-wishlistexistsbrowse').hide().removeClass('show').addClass('hide');//.attr('href', response.wishlist_url);
                    elwrap.find( '.yith-wcwl-wishlistaddedbrowse' ).hide().removeClass('show').addClass('hide');//.attr('href', response.wishlist_url);
                } 
